<template>
    <div class="container">
        ...
        <div class="main" ref="printContent" id="myPrint">
            <div class="text-center text-2xl pt-6">{{count}}以内连续减法</div>
...
const getEquation = (amount = 2, max = count.value) => {
    ...
    var a, b ,c;
    console.log(arr);
    a = Math.round(Math.random() * (max - max * 0.5) + max * 0.5)
    
    b = Math.round(Math.random() *  (a - a * 0.5) + a * 0.5)
    c = Math.round(Math.random() * (b - 1) + 1)
